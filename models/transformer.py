...
class Transformer(tf.keras.Model):
    def __init__(self, num_layers=2, d_model=512, num_heads=8, dff=2048, input_vocab_size=8500,
        ...
        if config is not None:
            ...
            input_vocab_size = config.model.fbanks
            target_vocab_size = config.model.vocab_size
            rate = config.model.drpout
        else:
...
if __name__=='__main__':
    ...
    temp_target = tf.keras.layers.Input((64,26),dtype=tf.float32)
    # 如果想对inputs浓缩，那么几个mask也要简历Input
